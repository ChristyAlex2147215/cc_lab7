...
router.get("/", function(request, response, next){
	...
	var query = "SELECT * FROM Covid_details ORDER BY No_of_Positive DESC";
	...
	database.query(query, function(error, data){
		...
		{
			response.render('sample_data', {title:'State-Wise COVID-19 Data  ', action:'list', sampleData:data, message:request.flash('success')});
		}
...
router.post("/add_sample_data", function(request, response, next){
	...
	const {State_Name,Date_of_Record,No_of_Positive,No_of_Samples,No_of_Deaths,No_of_Negative,No_of_Discharge} = request.body;
	...
	VALUES ("${State_Name}", "${Date_of_Record}", "${No_of_Positive}", "${No_of_Samples}", "${No_of_Deaths}", "${No_of_Negative}", "${No_of_Discharge}")
	`;
